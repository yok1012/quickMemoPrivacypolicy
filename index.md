# QuickMemo Privacy Policy / プライバシーポリシー
...
- 注意事項：サブスクリプションの解約はApp Storeのサブスクリプション管理から行ってください（アカウント削除はサブスクリプションの自動更新停止を代替しません）。
...
- Information provided by Sign in with Apple (user ID, email address, name) is used only for managing subscription rights
...
Permission grants/revocations can be changed at any time from iOS Settings app > [App Name]. Even if permissions are denied, basic memo functions remain available (however, calendar integration will not be available).
...
- Important: Please cancel subscriptions from App Store subscription management (account deletion does not automatically stop subscription renewal).
